...
*   **极致排版**: 日期列标题精简为 `M/D` 格式（如 `8/22`），备注列自动后置，确保核心财务数据始终处于视觉中心。
...
### 12. 同一 ISBN 多轮买卖支持 (2026-07-15)
*   引入系统字段 `记录ID`（每条持仓/交易记录唯一），解决“同一 ISBN 二次买入后二次卖出”被单行覆盖的问题。
*   当某 ISBN 的旧记录已售出后，再次出现在待售抓取结果时，会自动新建一条新记录（新 `记录ID`），保留历史已售记录不被覆盖。
*   `manual_overrides.csv` 会自动维护 `记录ID` 列；该列仅用于系统匹配，请勿手工修改。

### 13. 手工覆盖匹配与状态重算 (2026-09-02)
*   **跨表严格 ISBN 优先**：主表与 `manual_overrides.csv` 均优先使用 ISBN 精确匹配；只有两条记录都没有 ISBN 时才会按书名匹配。这样同名的平装、精装或不同版次不会互相带入购入价、售出价和备注。
*   **状态由业务字段推导**：同步时会按“已赠送/已丢弃 → 售出价格 → 购入价格 → 未持有”的顺序重算状态。填写售出价格必为“已售”，填写购入价格（包括 `0`）且未处置必为“持有”，避免新增记录保留过期的“未持有”状态。
...
*   **标记赠送/丢弃**：在 `状态` 或 `处理标签` 填 `已赠送` / `已丢弃`，不计卖出收入，但保留购入成本并计入已实现损益。
...
推荐列：
```csv
记录ID,ISBN,书名,状态,购入价格,售出价格,售出时间,处理标签,备注
```

`状态` 推荐值：`持有`、`未持有`、`已售`、`已赠送`、`已丢弃`、`已移除`。
`处理标签` 可选值：`待售`、`已看`、`已赠送`、`已丢弃`（用于运营分组）。
`记录ID` 为系统列，用于保证同一 ISBN 的多轮交易能被正确区分。

规则：
- 程序会优先按 `ISBN` 精确合并；若 `ISBN` 为空则回退按 `书名` 合并
- 每次同步会自动维护 `manual_overrides.csv`：首次自动初始化，后续自动补新书
- `书名` 会跟随主表更新，方便你识别目标行
- `购入价格` / `售出价格` / `售出时间` / `处理标签` / `备注` 以手工文件为准（强覆盖关系）；`状态` 会依据赠送/丢弃、售出价和购入价自动重算
- 手工文件中的上述字段会在同步时回写并持久化到主表 `inventory_auto.csv`（包含清空值）
- `处理标签` 建议用于运营分组：填 `待售` / `已看` 会进入“持有待处理”折叠面板；填 `已赠送` / `已丢弃` 会进入对应处置分类
...
* 详情页顶部的“历史最高价”优先读取 `price_history.csv` 中从主表迁移来的 `历史最高价` 字段；如果这个最高价不在当前价格序列里，只显示在顶部字段，不强行补到折线图中。
...
5. **后续维护方式**：
   - 自动行情来自多抓鱼；
   - 手工字段统一在 `manual_overrides.csv` 维护（状态/购入价/售出价/处理标签/备注），同步时会回写到主表。
