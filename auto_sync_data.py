...
def migrate_and_update_csv(books_data, capture_date, csv_path='inventory_auto.csv'):
    """更新 inventory_auto.csv，执行状态转换与草稿清理逻辑"""
...
<body>
    ...
    with open('report_auto.html', 'w', encoding='utf-8') as f: f.write(html)
...
if __name__ == "__main__":
    ...
    if books_data:
        ...
        # 获取变动前快照 (从 inventory_auto.csv 读取)
        old_prices = {}
        if os.path.exists('inventory_auto.csv'):
            try:
                with open('inventory_auto.csv', 'r', encoding='utf-8-sig') as f:
                    reader = csv.DictReader(f)
                    for r in reader:
